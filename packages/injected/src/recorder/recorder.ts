...
// Global reference for cleanup - stored on window to survive re-injection
declare global {
  interface Window {
    __pwRecorderInstance?: Recorder;
  }
}
...
export class Recorder {
  ...
  constructor(injectedScript: InjectedScript, options?: { recorderMode?: 'default' | 'api' }) {
    ...
    if (injectedScript.window.top === injectedScript.window && options?.recorderMode !== 'api') {
    ...
    injectedScript.window.__pwRecorderInstance = this;
  }
  ...
  cleanup(): void {
    ...
    // Remove stylesheet from adoptedStyleSheets
    ...
    this.document.body?.removeAttribute('data-pw-cursor');
    
    // Clear global reference
    if (this.injectedScript.window.__pwRecorderInstance === this) {
      delete this.injectedScript.window.__pwRecorderInstance;
    }
  }
  ...
  installListeners() {
    ...
    // In API mode, only install listeners needed by JsonRecordActionTool:
    // click, auxclick, contextmenu, input, keydown
    if (this._recorderMode === 'api') {
      ...
      // Skip highlight, overlay, stylesheet in API mode
      this._currentTool?.install?.();
      return;
    }
